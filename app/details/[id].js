          ...
          <View style={styles.section}>
            ...
            <View style={styles.horariosGrid}>
              {horarios.map(h => (
                <TouchableOpacity 
                  key={h} 
                  style={[styles.horarioChip, { backgroundColor: colors.surface }, dataSelecionada === h && { backgroundColor: colors.primary }]}
                  onPress={() => setDataSelecionada(h)}
                >
                  <Text style={[styles.horarioText, { color: colors.text }, dataSelecionada === h && { color: '#fff', fontWeight: 'bold' }]}>{h}</Text>
                </TouchableOpacity>
              ))}
            </View>
          </View>
...
const styles = StyleSheet.create({
  ...
  horarioChip: { paddingHorizontal: 16, paddingVertical: 10, borderRadius: 8 },
